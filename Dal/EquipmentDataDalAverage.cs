...
namespace Dal
{
    public static class EquipmentDataDalAverage
    {
        ...
        {
            string sql = string.Format("select a.Temperature,a.Humidity, a.AddTime from tb_DataAverage a where a.AddTime >= @start and a.AddTime <= @end");
        ...
        public static List<EquipmentData> GetListByMonth(string year, string month, string roomId)
        {
            ...
            {
                conn.Open();
                list = EquipmentDataDalAverage.GetList(conn, start, end);
            }
        ...
        public static List<EquipmentData> GetListByTime(string start, string end, string fileId)
        {
            ...
            foreach (var item in dateList)
            {
                ...
                {
                    conn.Open();
                    var ll = EquipmentDataDalAverage.GetList(conn, startTime, endTime);
                    list.AddRange(ll);
                }
            }
